...
跳過、只有真的有today's bars才會呼叫run_universe_for_date、Phase 2/Phase 3(FinMind
基本面+當沖/借券賣出收集+組裝)跟產業分類都會在run_universe_for_date之前跑、Phase 3
組裝結果會merge進同一份fundamentals_by_code、抓產業分類失敗不會擋住整個收集。"""
...
_NO_PHASE3 = {"status": "skipped", "reason": "FINMIND_TOKEN 未設定"}
...
class DispositionPredictionCollectorTests(unittest.TestCase):
    ...
    def _patches(self, *, industry=_INDUSTRY_BY_CODE, fundamentals=None, phase3=None, run_result=None):
        return (
            ...
            patch.object(collector, "collect_finmind_phase3", return_value=_NO_PHASE3),
            patch.object(collector, "build_fundamentals_by_code", return_value=fundamentals or {}),
            patch.object(collector, "build_phase3_by_code", return_value=phase3 or {}),
    ...
    def test_runs_once_when_bars_ready(self):
        industry_patch, fetch_patch, phase3_fetch_patch, build_patch, phase3_build_patch, run_patch = self._patches(
            fundamentals={"2330": {"pe_ratio": 20.0}},
            phase3={"2330": {"day_trading_prev_day_ratio_pct": 1.5}},
            run_result={"2330": []},
        )
        with (
            ...
            phase3_fetch_patch as phase3_fetch_mock,
            build_patch as build_mock,
            phase3_build_patch as phase3_build_mock,
        ...
        self.assertEqual(result["phase3"], _NO_PHASE3)
        industry_mock.assert_called_once_with()
        fetch_mock.assert_called_once_with("2026-09-23", ["2330"])
        phase3_fetch_mock.assert_called_once_with("2026-09-23", ["2330"])
        build_mock.assert_called_once_with("2026-09-23", {"2330"}, industry_by_code=_INDUSTRY_BY_CODE)
        phase3_build_mock.assert_called_once_with("2026-09-23", {"2330"})
        # Phase 3組裝結果merge進同一份fundamentals_by_code再傳給run_universe_for_date。
        run_mock.assert_called_once_with(
            "2026-09-23", codes={"2330"}, industry_by_code=_INDUSTRY_BY_CODE,
            fundamentals_by_code={"2330": {"pe_ratio": 20.0, "day_trading_prev_day_ratio_pct": 1.5}},
        )

    def test_phase3_field_for_code_missing_from_fundamentals_still_merges(self):
        """build_fundamentals_by_code跟build_phase3_by_code理論上涵蓋同一組codes，但
        就算某個代號只出現在其中一份(例如Phase 2查無資料被跳過)，merge也不該炸掉。"""
        industry_patch, fetch_patch, phase3_fetch_patch, build_patch, phase3_build_patch, run_patch = self._patches(
            fundamentals={}, phase3={"2330": {"day_trading_prev_day_ratio_pct": 1.5}}, run_result={"2330": []},
        )
        with (
            patch.object(collector, "daily_bars_storage_status", return_value={"lastTradeDate": "2026-09-23"}),
            patch.object(collector, "official_group_codes", return_value={"2330"}),
            industry_patch, fetch_patch, phase3_fetch_patch, build_patch, phase3_build_patch,
            run_patch as run_mock,
        ):
            collector.collect_once(now=datetime(2026, 9, 23, 14, 0, tzinfo=TW_TZ))
        run_mock.assert_called_once_with(
            "2026-09-23", codes={"2330"}, industry_by_code=_INDUSTRY_BY_CODE,
            fundamentals_by_code={"2330": {"day_trading_prev_day_ratio_pct": 1.5}},
        )
    ...
    def test_industry_fetch_failure_falls_back_to_empty_and_still_runs(self):
        with (
            ...
            patch.object(collector, "build_phase3_by_code", return_value={}),
    ...
    def test_skips_second_call_same_day(self):
        industry_p, fetch_p, phase3_fetch_p, build_p, phase3_build_p, run_p = self._patches()
        with (
            patch.object(collector, "daily_bars_storage_status", return_value={"lastTradeDate": "2026-09-23"}),
            industry_p, fetch_p, phase3_fetch_p, build_p, phase3_build_p,
            run_p as run_mock,
        ):
    ...
    def test_runs_again_on_new_day(self):
        ...
        with (
            patch.object(collector, "daily_bars_storage_status", return_value={"lastTradeDate": "2026-09-23"}),
            industry_p, fetch_p, phase3_fetch_p, build_p, phase3_build_p, run_p,
        ):
            collector.collect_once(now=datetime(2026, 9, 23, 14, 0, tzinfo=TW_TZ))
        industry_p, fetch_p, phase3_fetch_p, build_p, phase3_build_p, run_p = self._patches()
        with (
