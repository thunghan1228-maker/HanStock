...
function DaytradeEarlySellNotifier({
  ...
  const activeDate = centerMode === "today" ? (combinedTodaySignals[0]?.tradeDate ?? taipeiTradeDate()) : centerMode === "instantLarge" ? (instantLargeSignals[0]?.tradeDate ?? taipeiTradeDate()) : centerMode === "mainForce" ? (mainForceSignals[0]?.tradeDate ?? taipeiTradeDate()) : centerMode === "fourGate" ? (fourGateSignals[0]?.tradeDate ?? taipeiTradeDate()) : centerMode === "extraLargeSell" ? (extraLargeSellSignals[0]?.tradeDate ?? taipeiTradeDate()) : centerMode === "extraLargeBuy" ? (extraLargeBuySignals[0]?.tradeDate ?? taipeiTradeDate()) : centerMode === "largeForce" ? (largeForceSignals[0]?.tradeDate ?? todaySignals[0]?.tradeDate ?? selectedDate) : selectedDate;
  ...
  const popupPanel = !signalWindowMode && signalAlertsEnabled && popupSignals.length > 0 ? <aside
      ...
      >
        ...
        >
          <div>{popupExternalWindow && <p className="external-popup-help">拖曳視窗最上方標題列到外接螢幕；Win＋Shift＋←／→ 也可切換。請保持主頁開啟，訊號會持續同步。</p>}<span>{popupIsInstantLarge ? `族群瞬間大單｜${activeDate.replaceAll("-", "/")}` : popupIsExtraLargeSell ? `盤中特大賣單｜${activeDate.replaceAll("-", "/")}` : popupIsExtraLargeBuy ? `盤中特大買單｜${activeDate.replaceAll("-", "/")}` : popupIsLargeForce ? `🐋 盤中大戶力｜${activeDate.replaceAll("-", "/")}` : popupIsFourGate ? `四項精選即時訊號｜${activeDate.replaceAll("-", "/")}` : popupIsMainForce ? `主力累計即時訊號｜${activeDate.replaceAll("-", "/")}` : popupSignals.some((item) => item.demo) ? "歷史警示示範｜2026/08/14" : "盤中全部即時訊號｜09:00–13:30"}</span><strong>{popupIsInstantLarge ? `漲跌幅前 20 族群逐筆成交即時偵測｜${popupSignals.length} 則完整顯示｜${popupPinned ? "已釘選固定" : "移出或點外面自動收起"}` : popupIsExtraLargeSell ? `盤中大單賣出累計已達前日大單淨額｜${popupSignals.length} 則完整顯示｜${popupPinned ? "已釘選固定" : "移出或點外面自動收起"}` : popupIsExtraLargeBuy ? `盤中大單買進累計已達前日大單淨賣超｜${popupSignals.length} 則完整顯示｜${popupPinned ? "已釘選固定" : "移出或點外面自動收起"}` : popupIsLargeForce ? `全市場盤中大戶力達 ±12%｜${popupSignals.length} 則完整顯示｜${popupPinned ? "已釘選固定" : "移出或點外面自動收起"}` : popupIsFourGate ? `今日正式即時計算｜${popupSignals.length} 則完整顯示｜${popupPinned ? "已釘選固定" : "移出或點外面自動收起"}` : popupIsMainForce ? `主力累計 A～D 同步濾網｜${popupSignals.length} 則完整顯示｜${popupPinned ? "已釘選固定" : "移出或點外面自動收起"}` : popupSignals.some((item) => item.demo) ? "歷史示範，不列入今日正式警示紀錄" : `全部 ${popupSignalModes.size} 類訊號合併顯示｜共 ${popupSignals.length} 則｜${popupPinned ? "已釘選固定" : "移出或點外面自動收起"}`}</strong></div>
